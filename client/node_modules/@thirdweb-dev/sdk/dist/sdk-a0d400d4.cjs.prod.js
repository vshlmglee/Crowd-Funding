...
var contractPublisher = require('./contract-publisher-53ac9a56.cjs.prod.js');
...
class UserWallet {
  ...
  async requestFunds(amount) {
    ...
    if (chainId === contractPublisher.ChainId.Localhost || chainId === contractPublisher.ChainId.Hardhat) {
      const localWallet = new UserWallet(new ethers.ethers.Wallet(LOCAL_NODE_PKEY, contractPublisher.getChainProvider(chainId, this.options)), this.options);
...
class MultichainRegistry {
  constructor(network, storage) {
    ...
    QueryParams._defineProperty(this, "addContract", contractPublisher.buildTransactionFunction(async contract => {
      return await this.addContracts.prepare([contract]);
    }));
    QueryParams._defineProperty(this, "addContracts", contractPublisher.buildTransactionFunction(async contracts => {
    ...
    QueryParams._defineProperty(this, "removeContract", contractPublisher.buildTransactionFunction(async contract => {
      return await this.removeContracts.prepare([contract]);
    }));
    QueryParams._defineProperty(this, "removeContracts", contractPublisher.buildTransactionFunction(async contracts => {
      const deployerAddress = await this.registryRouter.getSignerAddress();
...
async function deployCreate2Factory(signer, options) {
  ...
  if (!factoryExists) {
    ...
    const valueToSend = contractPublisher.CUSTOM_GAS_FOR_CHAIN[networkId] ? ethers.BigNumber.from(contractPublisher.CUSTOM_GAS_FOR_CHAIN[networkId].gasPrice).mul(100000) : toWei("0.01");
...
async function deployWithThrowawayDeployer(signer, transactions, options) {
  ...
  const deployTxns = await Promise.all(transactionBatches.map(txBatch => {
    ...
    const deployer = new ethers.ethers.ContractFactory(contractPublisher.DEPLOYER_ABI, contractPublisher.DEPLOYER_BYTECODE).connect(signer).deploy(txBatch);
    return deployer;
  }));
...
class ContractFactory extends contractPublisher.ContractWrapper {
  ...
  constructor(factoryAddr, network, storage, _options) {
    ...
    QueryParams._defineProperty(this, "deploy", contractPublisher.buildTransactionFunction(async (contractType, contractMetadata, eventEmitter, version, options, onExecute) => {
      ...
      const salt = options?.saltForProxyDeploy ? ethers.ethers.utils.id(options.saltForProxyDeploy) : ethers.ethers.utils.formatBytes32String(blockNumber.toString());
      return contractPublisher.Transaction.fromContractWrapper({
    ...
    QueryParams._defineProperty(this, "deployProxyByImplementation", contractPublisher.buildTransactionFunction(async (implementationAddress, implementationAbi, initializerFunction, initializerArgs, eventEmitter, saltForProxyDeploy, onExecute) => {
      const encodedFunc = ethers.Contract.getInterface(implementationAbi).encodeFunctionData(initializerFunction, initializerArgs);
      const blockNumber = await this.getProvider().getBlockNumber();
      const salt = saltForProxyDeploy ? ethers.ethers.utils.id(saltForProxyDeploy) : ethers.ethers.utils.formatBytes32String(blockNumber.toString());
      return contractPublisher.Transaction.fromContractWrapper({
        contractWrapper: this,
        method: "deployProxyByImplementation",
        args: [implementationAddress, encodedFunc, salt],
...
class ContractRegistry extends contractPublisher.ContractWrapper {
  constructor(registryAddress, network, options) {
    ...
    QueryParams._defineProperty(this, "addContract", contractPublisher.buildTransactionFunction(async contractAddress => {
      return await this.addContracts.prepare([contractAddress]);
    }));
    QueryParams._defineProperty(this, "addContracts", contractPublisher.buildTransactionFunction(async contractAddresses => {
    ...
    QueryParams._defineProperty(this, "removeContract", contractPublisher.buildTransactionFunction(async contractAddress => {
      return await this.removeContracts.prepare([contractAddress]);
    }));
    QueryParams._defineProperty(this, "removeContracts", contractPublisher.buildTransactionFunction(async contractAddresses => {
      const deployerAddress = await this.getSignerAddress();
...
class ContractDeployer extends contractPublisher.RPCConnectionHandler {
  ...
  constructor(network, _options, storage) {
     ...
     */
    QueryParams._defineProperty(this, "deployNFTCollection", contractPublisher.buildDeployTransactionFunction(async (metadata, options) => {
     ...
     * @returns the address of the deployed contract
     */
    QueryParams._defineProperty(this, "deployNFTDrop", contractPublisher.buildDeployTransactionFunction(async (metadata, options) => {
     ...
     * @returns the address of the deployed contract
     */
    QueryParams._defineProperty(this, "deploySignatureDrop", contractPublisher.buildDeployTransactionFunction(async (metadata, options) => {
     ...
     * @beta
     */
    QueryParams._defineProperty(this, "deployMultiwrap", contractPublisher.buildDeployTransactionFunction(async (metadata, options) => {
     ...
     * @returns the address of the deployed contract
     */
    QueryParams._defineProperty(this, "deployEdition", contractPublisher.buildDeployTransactionFunction(async (metadata, options) => {
     ...
     * @returns the address of the deployed contract
     */
    QueryParams._defineProperty(this, "deployEditionDrop", contractPublisher.buildDeployTransactionFunction(async (metadata, options) => {
     ...
     * @returns the address of the deployed contract
     */
    QueryParams._defineProperty(this, "deployToken", contractPublisher.buildDeployTransactionFunction(async (metadata, options) => {
     ...
     * @returns the address of the deployed contract
     */
    QueryParams._defineProperty(this, "deployTokenDrop", contractPublisher.buildDeployTransactionFunction(async (metadata, options) => {
     ...
     * @returns the address of the deployed contract
     */
    QueryParams._defineProperty(this, "deployMarketplace", contractPublisher.buildDeployTransactionFunction(async (metadata, options) => {
     ...
     * @returns the address of the deployed contract
     */
    QueryParams._defineProperty(this, "deployMarketplaceV3", contractPublisher.buildDeployTransactionFunction(async (metadata, options) => {
     ...
     * @returns the address of the deployed contract
     */
    QueryParams._defineProperty(this, "deployPack", contractPublisher.buildDeployTransactionFunction(async (metadata, options) => {
     ...
     * @returns the address of the deployed contract
     */
    QueryParams._defineProperty(this, "deploySplit", contractPublisher.buildDeployTransactionFunction(async (metadata, options) => {
     ...
     * @returns the address of the deployed contract
     */
    QueryParams._defineProperty(this, "deployVote", contractPublisher.buildDeployTransactionFunction(async (metadata, options) => {
     ...
     */
    QueryParams._defineProperty(this, "deployBuiltInContract", contractPublisher.buildDeployTransactionFunction(async function (contractType, contractMetadata) {
     ...
     */
    QueryParams._defineProperty(this, "deployReleasedContract", contractPublisher.buildDeployTransactionFunction(async function (publisherAddress, contractName, constructorParams) {
     ...
     */
    QueryParams._defineProperty(this, "deployViaFactory", contractPublisher.buildTransactionFunction(async (factoryAddress, implementationAddress, implementationAbi, initializerFunction, initializerArgs, saltForProxyDeploy) => {
     ...
     * @param initializerArgs
     */
    QueryParams._defineProperty(this, "deployProxy", contractPublisher.buildDeployTransactionFunction(async (implementationAddress, implementationAbi, initializerFunction, initializerArgs) => {
     ...
     * @param options
     */
    QueryParams._defineProperty(this, "deployViaAutoFactory", contractPublisher.buildDeployTransactionFunction(async (publishMetadataUri, deployMetadata, signer, initializerFunction, paramValues, options) => {
     ...
     * @param chainId
     */
    QueryParams._defineProperty(this, "deployViaCustomFactory", contractPublisher.buildDeployTransactionFunction(async (constructorParamValues, deployMetadata, signer, chainId) => {
     ...
     * @param options
     */
    QueryParams._defineProperty(this, "deployContractFromUri", contractPublisher.buildDeployTransactionFunction(async (publishMetadataUri, constructorParamValues, options) => {
      const signer = this.getSigner();
     ...
     */
    QueryParams._defineProperty(this, "deployContractWithAbi", contractPublisher.buildDeployTransactionFunction(async (abi, bytecode, constructorParams) => {
      const signer = this.getSigner();
