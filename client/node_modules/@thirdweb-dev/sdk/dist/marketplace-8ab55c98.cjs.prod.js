...
var marketplaceAuction = require('./marketplace-auction-85988907.cjs.prod.js');
...
class Marketplace {
  ...
  constructor(network, address, storage) {
     ...
     * ```
     */
    QueryParams._defineProperty(this, "makeOffer", contractPublisher.buildTransactionFunction(async (listingId, pricePerToken, quantity) => {
      const listing = await this.contractWrapper.readContract.listings(listingId);
      if (listing.listingId.toString() !== listingId.toString()) {
        throw new contractPublisher.ListingNotFoundError(this.getAddress(), listingId.toString());
      }
     ...
     */
    QueryParams._defineProperty(this, "setBidBufferBps", contractPublisher.buildTransactionFunction(async bufferBps => {
     ...
     */
    QueryParams._defineProperty(this, "setTimeBufferInSeconds", contractPublisher.buildTransactionFunction(async bufferInSeconds => {
      await this.roles.verify(["admin"], await this.contractWrapper.getSignerAddress());
     ...
     */
    QueryParams._defineProperty(this, "allowListingFromSpecificAssetOnly", contractPublisher.buildTransactionFunction(async contractAddress => {
      const encoded = [];
     ...
     */
    QueryParams._defineProperty(this, "allowListingFromAnyAsset", contractPublisher.buildTransactionFunction(async () => {
      const encoded = [];
      const members = await this.roles.get("asset");
